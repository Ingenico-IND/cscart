...
//require_once ('worldline/WorldlinePayment.php');
...
if (defined('PAYMENT_NOTIFICATION')) {
	if ($mode == 'return') {
		if (isset($_POST['msg']) === true) {
			$worldlineResponse = explode('|', $_POST['msg']);

			$orderDate = explode(" ", trim($worldlineResponse[8]));
			$newOrderDate = date("Y-m-d", strtotime($orderDate[0]));

			$merchantOrderId = $_SESSION['merchant_order_id'];
			$strCurDate = date('d-m-Y');

			$orderlist = db_get_array('SELECT * FROM ?:order_worldline WHERE order_id = ?i', $merchantOrderId);
			...
			if (empty($orderlist)) {
				$insertdata = array(
					...
					'merchant_identifier' => $worldlineResponse[3],
					'tpsl_identifier' => $worldlineResponse[5],
					'mandate_no' => $worldlineResponse[13],
					'response' => serialize($worldlineResponse),
					'status' => $worldlineResponse[1],
					'dateAdded' => $newOrderDate
				);
				db_query('INSERT INTO ?:order_worldline ?e', $insertdata);
			} else {
				$updatedata = array(
					'user_id' => $_SESSION['auth']['user_id'],
					'total' => $_SESSION['cart']['total'],
				...
				$order_id = $merchantOrderId;
				db_query('UPDATE ?:order_worldline SET ?u WHERE order_id = ?i', $updatedata, $order_id);
			}

			if ($worldlineResponse[0] == '0300') {
				$arr_req = array(
					"merchant" => [
						"identifier" => $_SESSION['cart']['payment_method_data']['processor_params']['merchant_id']
					],
					"transaction" => ["deviceIdentifier" => "S", "currency" => $_SESSION['settings']['secondary_currencyC']['value'], "dateTime" => $strCurDate, "token" => $worldlineResponse[5], "requestType" => "S"]
				);
			...
			} elseif ($worldlineResponse[0] == '0398') {
			...
			$pp_response['transaction_id'] = $worldlineResponse[3];
			$pp_response['client_id'] = $worldlineResponse[5];
	...
	$url = fn_url("payment_notification.return?payment=worldline", AREA, 'current');
	...
	$(document).ready(function() {
		...
		var configJson = {
			...
			'features': {
				...
				'enableNewWindowFlow': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['enableNewWindowFlow'] == 1) {
											echo true;
										} else {
											echo false;
										} ?>', //for hybrid applications please disable this by passing false
				'enableAbortResponse': false,
				'enableExpressPay': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['enableExpressPay'] == 1) {
											echo true;
										} else {
											echo false;
										} ?>', //if unique customer identifier is passed then save card functionality for end  end customer
				'enableInstrumentDeRegistration': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['enableInstrumentDeRegistration'] == 1) {
														echo true;
													} else {
														echo false;
													} ?>', //if unique customer identifier is passed then option to delete saved card by end customer
				'enableMerTxnDetails': true,
				'hideSavedInstruments': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['hideSavedInstruments'] == 1) {
												echo true;
											} else {
												echo false;
											} ?>',
				'separateCardMode': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['separateCardMode'] == 1) {
											echo true;
										} else {
											echo false;
										} ?>'
			...
			'consumerData': {
				...
				'checkoutElement': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['embedPaymentGatewayOnPage'] == 1) {
										echo "#embedPaymentGatewayOnPage";
									} else {
										echo "";
									} ?>',
				'paymentMode': '<?php echo $_SESSION['cart']['payment_method_data']['processor_params']['paymentMode']; ?>',
				<?php if (!empty($paymentModeOrder)) { ?> 'paymentModeOrder': ['<?php echo $paymentModeOrder; ?>'],
				<?php } ?> 'merchantLogoUrl': '<?php echo $_SESSION['cart']['payment_method_data']['processor_params']['logoURL']; ?>', //provided merchant logo will be displayed
				...
				'saveInstrument': '<?php if ($_SESSION['cart']['payment_method_data']['processor_params']['saveInstrument'] == 1) {
										echo true;
									} else {
										echo false;
									} ?>',
