...
if ($mode == "offline") {
    ...
    $payment_data = db_get_array("SELECT * FROM ?:payment_processors WHERE processor = ?s", "Worldline");
...
if ($mode == "refund") {
    if (defined('AJAX_REQUEST')) {
        ...
        $responseHtml = '<table class="table table-bordered table-hover"><tr class="info"><th width="100%" colspan=2 style="background: #000000;color: #ffffff;">Refund Result</th></tr><tr><td style="color: #0388cc;font-weight: 900;">Merchant Code</td><td>' . $refundData["merchantCode"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Merchant Transaction Reference No</td><td>' . $refundData["merchantTransactionIdentifier"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">worldline Merchant Transaction ID</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["identifier"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Refund Identifier</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["refundIdentifier"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Amount</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["amount"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Message</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["errorMessage"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Status Code</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["statusCode"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Status Message</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["statusMessage"] . '</td></tr><tr><td style="color: #0388cc;font-weight: 900;">Date Time</td><td>' . $refundData["paymentMethod"]["paymentTransaction"]["dateTime"] . '</td></tr></table>';

        if ($status == "0400") {
            $order_data = db_get_array("SELECT * FROM ?:order_worldline WHERE tpsl_identifier = ?i", $token);
...
if ($mode == "reconcile") {
    ...
    if (defined('AJAX_REQUEST')) {
        ...
        $orderData = db_get_array("SELECT * FROM ?:order_worldline as oi JOIN ?:orders as o ON o.order_id = oi.order_id WHERE o.status IN ('O','F','D') AND dateAdded BETWEEN '" . $reconcileFromDate . "' AND '" . $reconcileToDate . "'");
        $successFullOrdersIds = [];
        if ($orderData != '') {
            foreach ($orderData as $order_data) {
                $order_id = $order_data['order_id'];
                $request_array = array(
                    "merchant" => array("identifier" => $adminData['merchant_id']),
                    "transaction" => array(
                        "deviceIdentifier" => "S",
                ...
                if ($status_code == '0300') {
                    ...
                    db_query('UPDATE ?:order_worldline SET ?u WHERE order_id = ?i', $updatedata1, $order_id);
                    db_query('UPDATE ?:orders SET ?u WHERE order_id = ?i', $updatedata, $order_id);
